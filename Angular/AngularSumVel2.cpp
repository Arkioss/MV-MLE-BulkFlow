...
/*
 Compile with:
 g++-4.9 -o ASV1 AngularSumVel2.cpp `gsl-config --cflags --libs` -O3
 ...
 */
 ...
 for (int i_angle=0; i_angle<Angle.size(); i_angle++)
 {
     ...
     save_vector_to_file(Bulk_flow,ROOT_DIR + "BulkFlow/" + SUB_DIR + "/SumWeight/Bulk_flows_sum_" + i_angle_read + ".txt");
 }
