...
/*
 ...
 Compile with:
 g++-4.9 -o SCDC0 SamplingCompletenessDataCreate2.cpp `gsl-config --cflags --libs` -O3
...
int main()
{
 ...
 for (int i_angle=0; i_angle<Angle.size(); i_angle++)
 {
     ...
     for (int i_rot=0; i_rot<n_rot; i_rot++)
     {
         ...
         cout << "Reading HoriRun with " << N_rows_hori[0] << " rows" << endl;
         Horizon_run = Read_to_2d_vector(ROOT_DIR + "BulkFlow/DataCommon/Hori1000_angle_0.dat", N_rows_hori[0], n_cols_hori);
         cout << "n: " << i_rot << endl;
