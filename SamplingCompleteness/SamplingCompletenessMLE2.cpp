...
/*
 Compile with:
 g++-4.9 -o SCMLE4 SamplingCompletenessMLE2.cpp `gsl-config --cflags --libs` -O3
...
int main()
{
 ...
 int n_sub = 50; //Number of galaxies used for each bulk flow calculation
 int n_draw = 2000; //Number of bulk flows calculated
 int n_rot = 80; //Number of rotations
...
for (int i=0; i<Bulk_flow.size(); i++)
{
 ...
 for (int i_rot=0; i_rot<n_rot; i_rot++)  
 {
     ...
     Hori_xyz = Read_to_2d_vector(ROOT_DIR + "BulkFlow/" + SUB_DIR + "/Hori_sub_cart_0.25_" + i_rot_str + ".txt", n_rows, n_cols);
     cout << "n: " << i_rot << endl;
     ...
     save_2dvector_to_file(Bulk_flow,ROOT_DIR + "BulkFlow/" + SUB_DIR + "/MLE/MLE_Bulk_flows_0.25_" + i_rot_str + "_50.txt");
 }
